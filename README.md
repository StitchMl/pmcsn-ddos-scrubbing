...
### 2-bis. Parametrizzazione aderente al caso reale (`src/scenarios.py`)
Il modello è stato reso **aderente al caso AISURU** e i parametri derivano dai numeri reali (fonti in §4), scalati a un **nodo rappresentativo** tractabile. Il transitorio d'attacco è modellato con **fasce temporali** (evento artificiale), non con NHPP.

Mappatura reale → modello (nodo di scrubbing):
| Grandezza | Reale (AISURU 2025) | Modello (per nodo) |
|---|---|---|
| Picco HTTP | ~205 Mrps globali; ~330 PoP ⇒ ~0,62 Mrps/PoP | λ₂_picco = **60.000 rps** (~7,5× la capacità) |
| Durata picco | 35–69 s (impulsivo) | fascia **Picco 60 s** (Normale 30 + Picco 60 + Mitig. 30) |
| Ispezione L7 | parse HTTP + TLS + match regole | E(S) = **0,5 ms** ⇒ μ_core = 2.000 rps |
| Capacità nodo | m moduli DPI | m=4 ⇒ **8.000 rps**, buffer **K=2.000** |
| Legittimo | traffico utenti del PoP | λ₁ = **4.000 rps** (ρ_nom = 0,5) |

**Giustificazione della scala ridotta:** per la **proprietà del fattore di scala** (vista a lezione) riscalando (λ,μ)→(aλ,aμ) restano invarianti ρ, P_loss e le popolazioni; cambiano solo i tempi assoluti. Quindi i risultati **adimensionali** (ρ, P_loss, rapporti, Δ% delle contromisure) valgono anche a scala reale; i valori assoluti sono "per nodo rappresentativo".

> 🔑 **Risultato chiave emerso dalla simulazione realistica:** durante il picco il buffer satura e **anche il traffico legittimo viene scartato (~87%)**, nonostante la priorità in servizio. Causa: lo scarto **drop-tail all'ammissione non è priority-aware**. Questo è il "difetto del sistema reale" da correggere nel modello migliorativo (Step 8): ammissione priority-aware / **buffer riservato o Fast-Track per la Classe 1** / autoscaling.
...
├── docs/
│   ├── schema-sistema.svg/.png   schema a code: base + contromisure
│   └── risultati.md              risultati delle simulazioni
└── src/
    ├── rngs.py              PRNG multi-stream (Leemis & Park)          ✅
    ├── rvgs.py              generatori di variabili aleatorie          ✅
    ├── simulator.py         Next-Event con fasce d'attacco (reale)     ✅
    ├── scenarios.py         parametri reali AISURU + scenari           ✅
...
```
Per eseguire la demo (mappatura reale + regime nominale + attacco a fasce):  `python src/simulator.py`  (dalla cartella della repo). Nota: la run dell'attacco richiede ~10 s.
